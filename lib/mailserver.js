...
//Save Emails of files
function persistEmailInFS(mail){
  var persistencePath = fs.realpathSync(folder);
  logger.log(persistencePath);
  var filename = path.join(persistencePath, mail.id + '.mail');
  fs.writeFile(filename, JSON.stringify(mail) , 'utf8', function(err) {
      if(err) {
          logger.log(err);
      } else {
          logger.log('Saved mail %s to file %s', mail.id, filename);
      }
  });
}
...
/**
 * Set folder for persisting mails
 */
mailServer.setPersistentMailFolder = function(f){
  folder = f;
...
/**
 * load existing mails from folder
 */
mailServer.loadMailsFromFolder = function(){
    var persistencePath = fs.realpathSync(folder);
      ...
      files.forEach(function(file){
        ...
        fs.readFile(filePath, 'utf8', function(err, data){
            if(err) {
              ...
              store.push(mail);
              fs.createReadStream(filePath).pipe(fs.createWriteStream(path.join(tempDir, file.replace('mail','eml'))));
            }
        });
      });
    });
